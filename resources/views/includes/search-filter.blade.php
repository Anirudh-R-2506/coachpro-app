        ...
        <div class="col-6">        
            <div class="d-flex justify-content-start">
                @foreach($buttons as $text => $button)
                    <a href="{{$button['url']}}" {{-- target="{{$button['target']}}" --}} class="{{$button['class']}}">
                        <i class="{{$button['icon']}}"></i>&nbsp;{{$text}}
                    </a>                
                @endforeach
            </div>
        </div>
    @endif
    <div class="col d-flex align-items-center search-bar" x-data="dataTable()" x-init="init()">
